...
from transformers import AdamW, OpenAIGPTTokenizer, WEIGHTS_NAME, CONFIG_NAME

from openai_gpt_triplehead import OpenAIGPTTripleHeadsModel

from utils import get_dataset, make_logdir
from unique_persona_tags import persona_tags_10 as NEW_PERSONA_TOKENS # get new tokens for each persona

NUM_CANDIDATE_PERSONAS = 4 # default number of persona candidates for each dialog is 4
MODEL_NAME  = 'triplehead_10'
SPECIAL_TOKENS = ["<bos>", "<eos>", "<speaker1>", "<speaker2>"] + NEW_PERSONA_TOKENS + ["<pad>"]
ATTR_TO_SPECIAL_TOKEN = {'bos_token': '<bos>', 'eos_token': '<eos>', 'pad_token': '<pad>',
                         'additional_special_tokens': ['<speaker1>', '<speaker2>'] + NEW_PERSONA_TOKENS}
MODEL_INPUTS = ["input_ids", "mc_token_ids", "persona_token_ids", "lm_labels", "mc_labels", "persona_labels", "token_type_ids"]
...
def get_data_loaders(args, tokenizer):
    ...
    personachat = get_dataset(tokenizer, args.dataset_path, args.dataset_cache, MODEL_NAME)

    global NUM_CANDIDATE_PERSONAS

    logger.info("Build inputs and labels")
    datasets = {"train": defaultdict(list), "valid": defaultdict(list)}
    for dataset_name, dataset in personachat.items():
        NUM_CANDIDATE_PERSONAS = len(dataset[0]["personality"])
        num_candidate_responses = len(dataset[0]["utterances"][0]["candidates"])
        if args.num_candidates > 0 and dataset_name == 'train':
            num_candidate_responses = min(args.num_candidates, num_candidate_responses)
        for dialog in dataset:
            personas = dialog["personality"].copy()
            for utterance in dialog["utterances"]:
                history = utterance["history"][-(2*args.max_history+1):]
                for j, candidate in enumerate(utterance["candidates"][-num_candidate_responses:]):
                    for i, persona in enumerate(personas):
                        correct_candidate = j == num_candidate_responses - 1
                        correct_persona = i == NUM_CANDIDATE_PERSONAS - 1
                        lm_labels = bool(correct_candidate and correct_persona)
                        if not correct_candidate and correct_persona:
                ...
                datasets[dataset_name]["mc_labels"].append(num_candidate_responses - 1)
                datasets[dataset_name]["persona_labels"].append(NUM_CANDIDATE_PERSONAS - 1)
                datasets[dataset_name]["n_candidates"] = num_candidate_responses - 1 + NUM_CANDIDATE_PERSONAS
    ...
    for dataset_name, dataset in datasets.items():
        ...
        for input_name in MODEL_INPUTS:
            ...
            if input_name != "mc_labels" and input_name != "persona_labels":
